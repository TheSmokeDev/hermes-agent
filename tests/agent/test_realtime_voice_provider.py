...
def test_shared_event_vocabulary_is_typed_and_provider_data_is_opaque() -> None:
    ...
    events = [
        ...
        InputSpeechStarted(
            item_id="provider-input",
            offset_ms=17,
            provider_data={"native": {"sequence": [1]}},
        ),
    ...
    assert events[1].provider_data == {"native": {"sequence": (1,)}}


def test_input_speech_started_is_frozen_and_exactly_validated() -> None:
    event = InputSpeechStarted(item_id=None, offset_ms=None)
    assert event.item_id is None
    assert event.offset_ms is None
    with pytest.raises(AttributeError):
        event.offset_ms = 1

    for invalid in ("", " padded", "padded ", "x" * (MAX_IDENTIFIER_LENGTH + 1)):
        with pytest.raises(ValueError, match="item_id"):
            InputSpeechStarted(item_id=invalid)
    for invalid in (True, False, 1.0, "1", -1):
        with pytest.raises((TypeError, ValueError), match="offset_ms"):
            InputSpeechStarted(offset_ms=invalid)


@pytest.mark.asyncio
async def test_input_speech_started_is_passive_event_not_interruption_authority() -> None:
    event = InputSpeechStarted(item_id="provider-input", offset_ms=0)
    session = _Session()
    session.event_values = [event]

    assert [value async for value in session.events()] == [event]
...
class _FailingCancellationSession(_Session):
    def __init__(self) -> None:
...
class _CloseCancelsCancellationSession(_ControlledCancellationSession):
    ...
    async def _close(self) -> None:
        ...
        await self.cancel_exited.wait()
        self.release_events.set()


class _FailedCloseCancelsCancellationSession(_ControlledCancellationSession):
    def __init__(self) -> None:
        super().__init__()
        self.cancel_released = asyncio.Event()
        self.surface_cancel = asyncio.Event()
    ...
    async def _cancel_response(self, response_id: str) -> None:
        ...
        await self.surface_cancel.wait()
        raise asyncio.CancelledError
...
@pytest.mark.asyncio
async def test_cancel_response_is_exact_capability_closed_and_replay_safe() -> None:
    unsupported = _Session()
    with pytest.raises(UnsupportedRealtimeCapability) as exc_info:
    ...
    for invalid in (None, True, 1, "", " padded", "padded ", "x" * 513):
        with pytest.raises((TypeError, ValueError), match="response_id"):
    ...
    await closed.close()
    with pytest.raises(RuntimeError, match="closed"):
        await closed.cancel_response("response")
    assert closed.cancelled_responses == []


@pytest.mark.asyncio
async def test_cancel_response_duplicate_capacity_and_in_flight_limits_precede_hook(
    monkeypatch,
) -> None:
    monkeypatch.setattr(
        realtime_voice_provider, "MAX_ACCEPTED_RESPONSE_CANCELLATION_TOMBSTONES", 2
    )
    session = _Session({RealtimeCapability.RESPONSE_CANCELLATION})
    ...
    with pytest.raises(ValueError, match="already accepted/sent"):
        await session.cancel_response("response-a")
    with pytest.raises(ValueError, match="replay tracking limit"):
        await session.cancel_response("response-c")
    assert session.cancelled_responses == ["response-a", "response-b"]

    monkeypatch.setattr(
        realtime_voice_provider, "MAX_IN_FLIGHT_RESPONSE_CANCELLATIONS", 1
    )
    controlled = _ControlledCancellationSession()
    first = asyncio.create_task(controlled.cancel_response("response-a"))
    await controlled.cancel_entered.wait()
    with pytest.raises(ValueError, match="already accepted/sent"):
        await controlled.cancel_response("response-a")
    with pytest.raises(ValueError, match="in-flight response cancellation limit"):
        await controlled.cancel_response("response-b")
    assert controlled.cancelled_responses == ["response-a"]
    controlled.release_cancel.set()
    await first


@pytest.mark.asyncio
async def test_cancelled_cancel_response_waiter_retains_send_and_late_success() -> None:
    ...
    with pytest.raises(asyncio.CancelledError):
        await waiter
    assert session._in_flight_response_cancellations == {"response"}
    with pytest.raises(ValueError, match="already accepted/sent"):
        await session.cancel_response("response")

    send_task = session._response_cancellation_tasks["response"]
...
@pytest.mark.asyncio
async def test_cancelled_cancel_response_waiter_late_failure_converges_when_events_blocked() -> None:
    session = _ControlledCancellationSession(RuntimeError("late cancellation failure"))
    events = session.events()
    receiving = asyncio.create_task(anext(events))
    await session.events_entered.wait()
    waiter = asyncio.create_task(session.cancel_response("response"))
    ...
    failure = await receiving
    assert isinstance(failure, SessionFailure)
    assert failure.code == "response_cancellation_failed"
    await asyncio.wait({send_task})
    assert session.closed_count == 1
    assert session._response_cancellation_tasks == {}
    assert session._in_flight_response_cancellations == set()
    assert session._accepted_response_cancellation_tombstones == {}


@pytest.mark.asyncio
async def test_spontaneous_provider_cancelled_response_cancellation_is_terminal() -> None:
    session = _ControlledCancellationSession(asyncio.CancelledError())
    events = session.events()
    receiving = asyncio.create_task(anext(events))
    await session.events_entered.wait()
...
@pytest.mark.asyncio
async def test_intentional_close_cancellation_of_response_cancel_hook_is_normal() -> None:
    session = _CloseCancelsCancellationSession()
    cancelling = asyncio.create_task(session.cancel_response("response"))
    await session.cancel_entered.wait()
...
@pytest.mark.asyncio
async def test_failed_close_intent_for_response_cancellation_stays_pinned_until_exit() -> None:
    session = _FailedCloseCancelsCancellationSession()
    cancelling = asyncio.create_task(session.cancel_response("response"))
    await session.cancel_entered.wait()
...
@pytest.mark.asyncio
async def test_old_response_cancellation_callback_cannot_clear_replacement() -> None:
    session = _Session({RealtimeCapability.RESPONSE_CANCELLATION})
    old = asyncio.get_running_loop().create_future()
    replacement = asyncio.get_running_loop().create_future()
    old.set_result(None)
...
async def test_optional_operations_fail_with_typed_capability_error() -> None:
    ...
    operations = [
        ...
        (
            session.cancel_response("response"),
