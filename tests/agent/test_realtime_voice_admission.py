...
class _MultiPermitHost:
    def __init__(self) -> None:
        ...
        self.revoke_calls: dict[str, int] = {}
        self.submit_count = 0
    ...
    async def revoke(self, permit: _Permit) -> None:
        self.revoke_calls[permit.value] = self.revoke_calls.get(permit.value, 0) + 1
        if self.states[permit.value] != "issued":
            raise RuntimeError(f"duplicate revoke: {permit.value}")
...
async def test_close_waits_for_all_revocations_before_reporting_one_failure() -> None:
    ...
    assert host.revoke_calls == {"permit-1": 1, "permit-2": 1}

    await admission.close()
    assert host.states == {"permit-1": "revoked", "permit-2": "revoked"}
    assert host.revoke_calls == {"permit-1": 2, "permit-2": 1}
