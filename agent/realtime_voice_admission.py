...
class FinalTranscriptAdmission(Generic[PermitT, ReceiptT]):
    ...
    async def _revoke_permits(
        self,
        permits: tuple[PermitT, ...],
        *,
        on_settled: Callable[[tuple[PermitT, ...]], None] | None = None,
    ) -> None:
        if not permits:
            return
        ...
        async def revoke_all() -> None:
            ...
            failed_permits = tuple(
                permit
                for permit, result in zip(permits, results, strict=True)
                if isinstance(result, BaseException)
            )
            if on_settled is not None:
                on_settled(failed_permits)
    ...
    async def close(self) -> None:
        async with self._close_lock:
            ...
            def retain_failed(failed: tuple[PermitT, ...]) -> None:
                self._pending_revocations = failed

            await self._revoke_permits(permits, on_settled=retain_failed)
